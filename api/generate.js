export default async function handler(req, res) {
  ...
  if (!API_KEY) {
    return res.status(500).json({ error: 'VIEWMAX_API_KEY not set in environment variables' });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorMsg.toLowerCase().includes('credit') || errorMsg.toLowerCase().includes('balance')) {
        return res.status(402).json({ error: 'Insufficient ViewMax credits. Please check your ViewMax account balance.' });
      }
